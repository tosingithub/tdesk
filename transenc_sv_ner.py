...
"""
Named Entity Recognition on Swedish SiC Dataset
...
parser.add_argument('--ofile1', type=str, default='output_sv_ner_val_sv_ft_model_w4s0h0.txt', help='output file')
parser.add_argument('--ofile2', type=str, default='output_sv_ner_test_sv_ft_model_w4s0h0.txt', help='output file 2')
...
parser.add_argument('--nlayers', type=int, default=8, help='number of nn.TransformerEncoderLayer in nn.TransformerEncoder')
...
parser.add_argument('--save', type=str, default='modeltrans_sv_ner_sv_ft_model_w4s0h0.pt', help='path to save the final model')
parser.add_argument('--onnx-export', type=str, default='', help='path to export the final model in onnx format')
parser.add_argument('--nhead', type=int, default=2, help='the number of heads - parallel attention heads')
...
parser.add_argument('--pretrained', type=str, default='false', help='use pretrained embeddings- false or true')
...
if __name__ == '__main__':
    ...
    files = glob.glob('datasv/*.conll')
    data = pd.concat([pd.read_csv(f, delimiter='\t', header=None, encoding="utf-8").fillna(method="ffill") for f in files], ignore_index=True)
    #data = pd.read_csv("data/5089-62681.conll", delimiter='\t', header=None, encoding="utf-8").fillna(method="ffill")
    data.columns = ['C0', 'Word', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9', 'Tag', 'Type', 'C12']
    data['Type'] = data['Type'].replace('_', '', regex=True)  # switch _ to blank
    data['Type'] = data['Type'].replace('[0-9-:.]+', '', regex=True)  # remove numbers in wrong column due to reading
    data['Tag'] = data['Tag'].replace('_', 'O', regex=True)  # switch _ to 'O' for wrong data entry discovered in files
    df1 = data[data.C0 == 1]                # split based on value 1 in file/dataframe to identify sentence start point
    ind_token = 1
    indlist, sentences, labels, labs = [], [], [], []
    data['Tagtype'] = data.Tag + data.Type              # combine to a single, extra column to use as labels
    all_tokens = data['Word'].values
    all_labels = data['Tagtype'].values
    indlist = df1.index                                 # list of the starting points of each sentence token
    for ind in df1.index:
        if ind_token < len(indlist)-1:
            sentences.append(" ".join(all_tokens[ind:indlist[ind_token]]))      # make sentences from tokens
            labs = list(all_labels[ind:indlist[ind_token]])                     # make list of labels per sentence
            labels.append(labs)                                                 # make list of lists of labels
            ind_token += 1
        else:
            ind_token = len(data)
            sentences.append(" ".join(all_tokens[ind:ind_token]))               # add final sentence
            labs = list(all_labels[ind:ind_token])                                    # make list of labels per sentence
            labels.append(labs)                                                 # make list of lists of labels
    tags_vals = list(set(data["Tagtype"].values))                           # generate set of unique labels
    vocab = list(set(data["Word"].values))                              # generate vocab/unique data vales
    temp = []
    for tem in vocab:                                                   # split 2 or more entries, if any & add to vocab
        if " " in tem:
            temp = tem.split()
            for i in temp:
                vocab.append(i)
        temp = []
    vocab = list(set(vocab))                                            # re-do vocab to be unique list
    ...
    if args.pretrained == 'true':
        ...
        pretrained_model = fasttext.load_facebook_vectors("sv_ft_model_w4s0h0.bin")
    ...
    optimizer = torch.optim.RMSprop(model.parameters(), lr=args.lr) #, betas=(0.7, 0.99))
    ...
    for exp in range(1, args.nfullexp + 1):
        ...
        if args.sigopt == 'true':       # SigOpt experiment for optimization
            conn, sigoptexp = connfunc("transformerencoder_sv_ner")
            optimizers = {
